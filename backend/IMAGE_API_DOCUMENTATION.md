...
本文档介绍小红书Agent系统的图像生成、图像编辑和图生图功能API。这些功能基于火山引擎（字节跳动申图）的doubao-seedream-4.5模型实现。
...
1. **图像URL要求**: 用于编辑和图生图的图像URL必须是可公开访问的，建议使用CDN链接或Base64编码
2. **同步处理**: 火山引擎API是同步返回的，直接返回图片URL，无需轮询
3. **超时设置**: API调用超时时间为60秒，图生图功能为180秒
4. **模型选择**: 
   - 所有图像生成功能统一使用 `doubao-seedream-4-5-251128` （Doubao-Seedream-4.5版本）
   - 文生图、图像编辑、图生图均使用相同模型
...
- **2026-01-21**: 将图像生成服务从胜算云迁移到火山引擎，升级模型版本从4.0到4.5
